    ...
    while true do
		...
		if LocalPlayer.state['isLoggedIn'] then
			...
			if isLoggedIn and incinematic == false and inBathing == false and inClothing == false and showUI and Config.ShowInfoHud == true then
				DrawTxt("ID : "..tonumber(GetPlayerServerId(PlayerId())).."  - Time : "..string.format("%0.2d", GetClockHours())..":"..string.format("%0.2d", GetClockMinutes()), 0.03, 0.917, 0.4, 0.4, true, 255, 255, 255, 255, true)
			end
		else
			Wait(3000)
		end
...
RegisterNetEvent('jkt-hud:client:toggleMaps', function()
